...
<script>, with a <!--CORE--> marker where the shared theme engine belongs.
...
# page -> (url path, meta description, social image theme)
ROUTES = {
    "index.html":     ("",            "Two redesigned landing pages for omarchy.org, both live and theme-switchable across all 22 Omarchy themes."),
    "tty.html":       ("tty/",        "Omarchy's landing page as a terminal session: commands type themselves, output decodes out of ASCII noise, and an fzf overlay repaints the page in any of 22 themes."),
    "broadside.html": ("broadside/",  "Omarchy's landing page as an editorial poster: full-bleed theme wallpaper, oversized type, and a filmstrip of all 22 themes along the bottom edge."),
}
...
def main():
    ...
    for page in names:
        ...
        # index.html carries no theme engine, so the marker is simply absent
